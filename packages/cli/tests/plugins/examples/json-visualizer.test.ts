...
describe('JSON Visualizer Plugin', () => {
  ...
  it('should register the JSON visualizer', () => {
    ...
    expect(mockContext.registerResponseVisualizer).toHaveBeenCalledWith('json', expect.any(Function));
  });
  ...
  it('should visualize JSON response', () => {
    ...
    const visualizer = (mockContext.registerResponseVisualizer as unknown as ReturnType<typeof vi.fn>).mock.calls[0][1];
