...
describe('Markdown Formatter Plugin', () => {
  ...
  it('should register the markdown formatter', () => {
    ...
    expect(mockContext.registerOutputFormatter).toHaveBeenCalledWith('markdown', expect.any(Function));
  });
  ...
  it('should format JSON response as markdown', () => {
    ...
    const formatter = (mockContext.registerOutputFormatter as unknown as ReturnType<typeof vi.fn>).mock.calls[0][1];
