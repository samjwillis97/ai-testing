...
describe('Help Command Plugin', () => {
  ...
  it('should handle help command execution', () => {
    ...
    const commandHandler = (mockContext.registerCommand as unknown as ReturnType<typeof vi.fn>).mock
      .calls[0][1];
    ...
    expect(
      consoleLogSpy.mock.calls.some((call) => call[0].includes('SHC CLI - Extended Help'))
    ).toBe(true);
