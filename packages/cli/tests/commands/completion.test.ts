...
beforeEach(async () => {
  ...
  fs.writeFileSync(
    path.join(TEST_COLLECTION_PATH, 'get-users.json'),
    JSON.stringify({
      method: 'GET',
  ...
  fs.writeFileSync(
    path.join(TEST_COLLECTION_PATH, 'create-user.json'),
    JSON.stringify({
      method: 'POST',
      url: 'https://api.example.com/users',
      headers: {},
...
describe('Completion Command', () => {
  ...
  describe('Shell Completion Generation', () => {
    it('should generate bash completion script', async () => {
      ...
      vi.spyOn(completionUtils, 'generateCompletionScript').mockReturnValue(
    ...
    it('should handle errors during script generation', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Failed to generate completion script: Test error'
      );
      expect(processExitSpy).toHaveBeenCalledWith(1);
  ...
  describe('Collection and Request Completion', () => {
    it.skip('should list collections for completion', async () => {
      ...
      vi.spyOn(completionUtils, 'getCollectionsForCompletion').mockResolvedValue([
        'collection1',
        'collection2',
      ]);
    ...
    it.skip('should handle errors when listing collections', async () => {
      ...
      vi.spyOn(completionUtils, 'getCollectionsForCompletion').mockRejectedValue(
    ...
    it.skip('should list requests for completion', async () => {
      ...
      vi.spyOn(completionUtils, 'getRequestsForCompletion').mockResolvedValue([
        'request1',
        'request2',
      ]);
      ...
      expect(completionUtils.getRequestsForCompletion).toHaveBeenCalledWith(
        'test-collection',
        expect.anything()
      );
    ...
    it.skip('should handle errors when listing requests', async () => {
      ...
      vi.spyOn(completionUtils, 'getRequestsForCompletion').mockRejectedValue(
        new Error('Test error')
      );
