...
`;
      ...
      await expect(configManager.loadFromFile('/path/to/config.yaml')).rejects.toThrow(
        'Invalid configuration: core.http.timeout: Expected number'
      );
    });
  });
...
`;
  ...
  describe('validateConfig method', () => {
    it('should validate HTTP configuration with various edge cases', async () => {
      ...
      await expect(configManager.validateConfig(invalidTimeoutConfig)).rejects.toThrow(
        /HTTP timeout must be a number/
      );
      ...
      await expect(configManager.validateConfig(invalidRedirectsConfig)).rejects.toThrow(
        /HTTP max_redirects must be a number/
      );
      ...
      await expect(configManager.validateConfig(invalidRetryAttemptsConfig)).rejects.toThrow(
        /HTTP retry attempts must be a number/
      );
      ...
      await expect(configManager.validateConfig(invalidRetryBackoffConfig)).rejects.toThrow(
        /HTTP retry backoff must be a string/
      );
    });
    ...
    it('should validate logging configuration with various edge cases', async () => {
      ...
      await expect(configManager.validateConfig(invalidLevelConfig)).rejects.toThrow(
      ...
      await expect(configManager.validateConfig(invalidFormatConfig)).rejects.toThrow(
        /Logging format must be a string/
      );
      ...
      await expect(configManager.validateConfig(invalidOutputConfig)).rejects.toThrow(
        /Logging output must be a string/
      );
    });
    ...
    it('should validate storage configuration with various edge cases', async () => {
      ...
      await expect(configManager.validateConfig(invalidTypeConfig)).rejects.toThrow(
        /Storage collections type must be a string/
      );
      ...
      await expect(configManager.validateConfig(invalidPathConfig)).rejects.toThrow(
        /Storage collections path must be a string/
      );
    });
  });
  ...
  describe('resolveObject method with edge cases', () => {
    ...
    it('should handle complex nested objects with arrays', async () => {
      ...
      const complexObj: Record<string, unknown> = {
        ...
        nested: {
          value: '${math.add(7, 8)}',
          array: [{ calc: '${math.add(9, 10)}' }, { calc: '${math.add(11, 12)}' }],
        },
      };
      ...
      expect(
        ((resolved as { nested?: { array?: unknown[] } }).nested?.array?.[0] as { calc?: string })
          .calc
      ).toBe('19');
      expect(
        ((resolved as { nested?: { array?: unknown[] } }).nested?.array?.[1] as { calc?: string })
          .calc
      ).toBe('23');
    });
  });
  ...
  describe('saveToFile', () => {
    it('should save configuration to YAML file', async () => {
      ...
      expect(fs.writeFile).toHaveBeenCalledWith('/path/to/config.yaml', expect.any(String), 'utf8');
    });

    it('should save configuration to JSON file', async () => {
      await configManager.saveToFile('/path/to/config.json');

      expect(fs.writeFile).toHaveBeenCalledWith('/path/to/config.json', expect.any(String), 'utf8');
    });

    it('should throw error for unsupported file type', async () => {
      await expect(configManager.saveToFile('/path/to/config.txt')).rejects.toThrow(
        'Unsupported file type'
      );
    });
  });
  ...
  describe('secret management', () => {
    ...
    it('should throw error if secret not found', async () => {
      await expect(configManager.getSecret('NONEXISTENT')).rejects.toThrow(
